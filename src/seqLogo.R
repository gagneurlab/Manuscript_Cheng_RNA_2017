...
## plot sequence logo
##
## This code adapted from the seqLogo package from Bembom O
##
######################
...
seqLogo <- function(pwm, ic.scale=TRUE, xaxis=TRUE, yaxis=TRUE, xfontsize=15, yfontsize=15,fold=1,xaxislabel=c(1:ncol(pwm)),shrink=c()){
  require(grid)
  if (class(pwm) == "pwm"){
    pwm <- pwm@pwm    
  ...
  if (any(abs(1 - apply(pwm,2,sum)) > 0.05))
  ...
  grid.polygon(x=unit(letters$x,"native"), y=unit((letters$y)*fold,"native"),
               id=letters$id,
               gp=gpar(fill=letters$fill,col="transparent"))
  if (xaxis){
    grid.xaxis(at=seq(0.5,ncol(pwm)-0.5),label=xaxislabel, gp=gpar(fontsize=xfontsize))
    grid.text("Position",y=unit(-3,"lines"), gp=gpar(fontsize=xfontsize))
  }
  if (yaxis){
    grid.yaxis(at=seq(0,2,0.1*fold), label=seq(0,2/fold,0.1), gp=gpar(fontsize=yfontsize))
